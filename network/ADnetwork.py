...
class ADEncoder(nn.Module):
    ...
    def __init__(self, isize=64, nz=100, nc=3, ndf=64, ngpu=1, n_extra_layers=0, add_final_conv=True):
        ...
        self.isize = isize
        #Alister add down0 2023-01-16
        self.conv0 = nn.Conv2d(nc, nc, 4, 1, padding ="same",bias=False) #isize/2
        ...
        self.attn1 = Self_Attn(64,8,'relu')
        ...
        main = nn.Sequential()
        #Alister add down0 2023-01-16
        main.add_module('pyramid-UNetDown-{0}-{1}'.format(nc,  nc),self.conv0)
        main.add_module('pyramid-UNetDown-{0}-{1}'.format(nc,  64),self.down1)
        #main.add_module('pyramid-SelfAttention-{0}-{1}-{2}'.format(64,  8,'relu'),self.attn1)
        main.add_module('pyramid-UNetDown-{0}-{1}'.format(64, 128),self.down2)
        #main.add_module('pyramid-SelfAttention-{0}-{1}-{2}'.format(128,  8,'relu'),self.attn2)
    ...
    def forward(self, input, attention):
        ...
        d0 = self.conv0(input)
        #2023-01-16 implement  pixelwise add  feature = feature + attention
        d0 = d0 + attention
        d1 = self.down1(d0) #16x16,64
        #d1a,attn1 = self.attn1(d1) #16x16,64
    
        d2 = self.down2(d1) #8x8,128
        #d2a,attn2 = self.attn2(d2) #8x8,128
        
        d3 = self.down3(d2) #4x4,256
        #if self.isize == 32:
            #output = self.conv1(d3)
...
class ADDecoder(nn.Module):
    ...
    def __init__(self,isize=64, nz=100, nc=3, ngf=64, ngpu=1, n_extra_layers=0):
        ...
        if isize==128:
            ...
            self.up3_for_attn = nn.Sequential(nn.Upsample(scale_factor=2),nn.Conv2d(128, 3, 3, padding=1))
        '''
...
class ADNetG(nn.Module):
    ...
    def __init__(self, isize=64, nc=3, nz=100, ngf=64, ndf=64, ngpu=1, extralayers=0):
        ...
        self.encoder1 = ADEncoder(self.isize, self.nz, self.nc, self.ngf, self.ngpu, self.extralayers)
        self.decoder = ADDecoder(self.isize, self.nz, self.nc, self.ngf, self.ngpu, self.extralayers)
        self.encoder2 = ADEncoder(self.isize, self.nz, self.nc, self.ngf, self.ngpu, self.extralayers)

    def forward(self, x, attention):
        d, latent_i = self.encoder1(x, attention)
        gen_imag, gen_attn = self.decoder(latent_i,d)
        d, latent_o = self.encoder2(gen_imag,gen_attn)
        return gen_imag, latent_i, latent_o, gen_attn
    

class ADNetD(nn.Module):
    """
    DISCRIMINATOR NETWORK
    """

    def __init__(self,isize=64, nc=3):
        super(ADNetD, self).__init__()
        #model = SSAEncoder(isize=isize, nz=1, nc=nc, ndf=64, ngpu=1, n_extra_layers=0)
        model_fusion = ADEncoder(isize=isize, nz=1, nc=nc, ndf=64, ngpu=1, n_extra_layers=0)

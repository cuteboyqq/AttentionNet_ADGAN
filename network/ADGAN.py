...
from network.ADnetwork import ADNetG, ADNetD, weights_init
...
class ADGanomaly(nn.Module):
    ...
    def __init__(self,args):
        ...
        super(ADGanomaly, self).__init__()
        ...
        self.resume_adgan = args.weights_adgan
        self.nz = args.nz
        self.nc = args.nc
        ...
        self.netg = ADNetG(self.isize,self.nc,self.nz).to(self.device)
        self.netd = ADNetD(self.isize,self.nc).to(self.device)
        #Alister add 2023-01-14
        #self.netd2 = SSANetD2(self.isize,self.nc*2).to(self.device)
        ...
        if self.resume_adgan != '':
            ...
            self.netg.load_state_dict(torch.load(os.path.join(self.resume_adgan, 'best_adnetG.pt')))
            self.netd.load_state_dict(torch.load(os.path.join(self.resume_adgan, 'best_adnetD.pt')))
            #self.netd2.load_state_dict(torch.load(os.path.join(self.resume, 'netD2.pt')))
            print("\tDone.\n")
        ...
        #Alister 2023-01-14
        #self.optimizer_d2 = optim.Adam(self.netd2.parameters(), lr=self.lr, betas=(self.beta1, 0.999))
        self.optimizer_g = optim.Adam(self.netg.parameters(), lr=self.lr, betas=(self.beta1, 0.999))

    ##
    def forward_g(self,x, attention):
        """ Forward propagate through netG
        """
        self.fake, self.latent_i, self.latent_o, self.fake_attn = self.netg(x,attention)
        #self.x2.copy_(x)
    ...
    ##
    def cutout(self, im, p=1.0, rotation=False):
    # Applies image cutout augmentation https://arxiv.org/abs/1708.04552
        ...
        #im = torch.tensor(im) #this code is not work

...
from network.ADGAN import ADGanomaly
...
def train(args): 
    ...
    AD_Ganomaly = ADGanomaly(args)
    #print(Skip_SelfAttention_Ganomaly)
    ''' train epochs'''
    train_epochs(Skip_SelfAttention_Ganomaly, AD_Ganomaly, train_loader,test_loader,args)


def train_epochs(model, AD_model, train_loader,test_loader,args):
    ...
    for epoch in range(1, args.epoch+1):
        ...
        for images, _  in pbar: 
            ...
            error_g, error_d, fake_img, model_g, model_d, error_g_attn, error_g_ano_attn, fake_attn = outputs
            loss = error_g + error_d
            
            bar_str = ' epoch:{} AttentionNet: loss_g:{:.6f} loss_d:{:.6f} loss_g_attn:{:.6f} error_g_ano_attn:{:.6f}'.format(epoch,error_g.detach().cpu().numpy(),
            ...
            if ADGAN:
                ...
                print("fake_attn: {}".format(fake_attn.shape))
                ADGAN_outputs = AD_model(images,fake_attn)
                error_g, error_d, fake_img = ADGAN_outputs
                
                bar_str_adgan = 'epoch:{} ADGAN: loss_g:{:.6f} loss_d:{:.6f} '.format(epoch,error_g.detach().cpu().numpy(),
                                                                         error_d.detach().cpu().numpy(),
                                                                        )
                PREFIX_ADGAN = color.colorstr(bar_str_adgan)
                pbar.desc = f'{PREFIX_ADGAN}'
        #train_loss = train_loss/len(train_loader)
        
        color_str = 'Epoch: {} \t AttentionNet: Avg_G_loss: {:.6f} Avg_D_loss: {:.6f}'.format(epoch, avg_g_loss,avg_d_loss)
        ...
        #Start calculate auc at apoch=2
        if epoch>1:
            #calculate auc
...
def get_args():
    ...
    parser.add_argument('-imgtestdir','--img-testdir',help='val dataset',default=r"C:\factory_data\2022-12-30\test")
    ...
    parser.add_argument('-savedir','--save-dir',help='save model dir',default=r"./runs/train/2023-01-15/32-nz100-ngf64-ndf64-SkipCBAM-SelfAttention-Ganomaly")
    parser.add_argument('-weights','--weights',help='save model dir',default=r"")
    parser.add_argument('-weightsadgan','--weights-adgan',help='save model dir',default=r"")
